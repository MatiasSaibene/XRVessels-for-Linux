...
PopupHUDArea::~PopupHUDArea()
{
    ...
    oapiReleaseBrush(m_hBackgroundBrush);
}

// set main HUD color
void PopupHUDArea::SetColor(uint32_t color)
{
    ...
    if (color != m_colorRef)
    {
        ...
        m_pen0 = oapiCreatePen(1, 1, m_colorRef);
    }
}

void PopupHUDArea::SetBackgroundColor(uint32_t color)
{
    ...
    if (color != m_bgColorRef)
    {
        ...
        m_hBackgroundBrush = oapiCreateBrush(color);
    }
}
...
bool PopupHUDArea::Redraw2D(const int event, const SURFHANDLE surf)
{
    ...
    if (m_topYCoordinate < m_height) // is HUD not OFF; i.e., is the top of the HUD visible?
    {
        ...
        retVal = DrawHUD(event, m_topYCoordinate, skp, m_colorRef, forceRender);
        ...
        if (retVal)
        {
            ...
            skp->MoveTo(0, m_height);                 // bottom-left corner  
            skp->LineTo(0, m_topYCoordinate);

            skp->MoveTo(0, m_topYCoordinate);           // top-left corner
            skp->LineTo(m_width, m_topYCoordinate);

            skp->MoveTo(m_width - 1, m_topYCoordinate);   // top-right corner
            skp->LineTo(m_width - 1, m_height);
