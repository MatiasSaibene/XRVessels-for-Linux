...
#define ADD_SWITCH_AND_LED(switchClass, x, aidSwitch, aidLED, refLedIsOn)        \
    ...
    AddArea(new LEDArea    (*this, _COORD2( x-1, ledY   ), aidLED, GetXR5().refLedIsOn))
...
#define ADD_SWITCH_AND_INDICATOR(switchClass, x, aidSwitch, aidIndicator, doorStatus, indicatorSurfaceIDB, animationState)  \
    ...
    AddArea(new DoorIndicatorArea(*this, _COORD2( x-9, indicatorY), aidIndicator, VCPANEL_TEXTURE_NONE, &GetXR5().doorStatus, indicatorSurfaceIDB, &GetXR5().animationState))

#define ADD_SUPPLY_SWITCH_AND_LED(x, aidSwitch, aidLED, refSwitchState, refPressure)                                                              \
    AddArea(new ExtSupplyLineToggleSwitchArea(*this, _COORD2( x,   switchY), aidSwitch, aidLED, GetXR5().refSwitchState, GetXR5().refPressure));  \
    AddArea(new LEDArea                      (*this, _COORD2( x-1, ledY   ), aidLED, GetXR5().refSwitchState))

//----------------------------------------------------------------------

// base panel for the XR5 that all of our panels extend
...
void XR5InstrumentPanel::InitMDA(MultiDisplayArea *pMDA)
{
    ...
    pMDA->AddDisplayMode(new XR5HullTempsMultiDisplayMode   (MDMID_HULL_TEMPS));
    ...
    pMDA->AddDisplayMode(new XR5ReentryCheckMultiDisplayMode(MDMID_REENTRY_CHECK));
}
...
bool XR5MainInstrumentPanel::Activate() 
{
    ...
    GetXR5().m_pMDA = static_cast<MultiDisplayArea *>(GetArea(AID_MULTI_DISPLAY));

    GetXR5().SetMDAModeForCustomAutopilot();  // update the MDA mode if MDA is visible
...
bool XR5PayloadInstrumentPanel::Activate()
{
    ...
    GetXR5().ResetCameraToPayloadBay();

    // enable the external mesh to be rendered in the external pass so that cargo containers may obscure it
    GetVessel().SetMeshVisibilityMode(GetXR5().m_exteriorMeshIndex, (MESHVIS_EXTERNAL | MESHVIS_COCKPIT | MESHVIS_EXTPASS));
...
void XR5PayloadInstrumentPanel::Deactivate()
{
    // only reset if the panel is currently active
    if (IsActive())
    {
        // reset the mesh visibility to normal
        GetVessel().SetMeshVisibilityMode(GetXR5().m_exteriorMeshIndex, MESHVIS_EXTERNAL);
    ...
    // do the work
    XR5InstrumentPanel::Deactivate();
}

// add areas common for all panel resolutions    
void XR5PayloadInstrumentPanel::AddCommonAreas()
{
...
// add areas common for all panel resolutions    
void XR5OverheadInstrumentPanel::AddCommonAreas()
{
    // add components
...
void XR5UpperInstrumentPanel::Add1600PlusAreas(const int width)
{
    // calibrated for 1600
    const int shift = (width - 1600) / 2;

    // add components
    AddComponent(new XR5WarningLightsComponent(*this, _COORD2(shift +   71,  63)));
...
void XR5UpperInstrumentPanel::AddCommonAreas(const int width)
{
    ...
    AddComponent(new XR5ActiveEVAPortComponent   (*this, _COORD2(shift + 281,  249)));
    ...
    AddArea(new XR5CrewDisplayArea                  (*this, _COORD2(shift + 236, 186), AID_CREW_DISPLAY));
    AddArea(new SystemsDisplayScreen                (*this, _COORD2(shift + 867, 178), AID_SYSTEMS_DISPLAY_SCREEN));
}
...
bool XR5LowerInstrumentPanel::Activate()
{
    ...
    oapiRegisterPanelBackground (m_hBmp, PANEL_ATTACH_TOP | GetXR5().GetLowerPanelMoveoutFlag());  // do NOT attach at the bottom; this will prevent stretching
...
// width = panel width
void XR5LowerInstrumentPanel::AddCommonAreas(const int width)
{
    ...
    AddComponent(new XR5WarningLightsComponent    (*this, _COORD2(shift + 1057,  90)));
    ...
    AddArea(new DoorMediumLEDArea      (*this, _COORD2(shift + 1393, 323), AID_RADIATOR_DEPLOYED_LED, GetXR5().radiator_status, true));  // redraw always
...
// add areas common to 1600-pixel-wide or wider panels
// width = panel width
void XR5LowerInstrumentPanel::Add1600PlusAreas(const int width)
{
    const int shift = (width - 1600) / 2;   // calibrated below for 1600 pixels, but the end result is the same
...
// Constructor
// vessel = our parent vessel
XR5MainInstrumentPanel1280::XR5MainInstrumentPanel1280(XR5Vanguard &vessel) : 
    XR5MainInstrumentPanel(vessel, IDB_PANEL1_1280)
{
    // create our components
    ...
    AddComponent(new XR5WarningLightsComponent    (*this, _COORD2(1040, 159)));
...
// Constructor
// vessel = our parent vessel
XR5UpperInstrumentPanel1280::XR5UpperInstrumentPanel1280(XR5Vanguard &vessel) : 
    XR5UpperInstrumentPanel(vessel, IDB_PANEL2_1280)
{
    AddCommonAreas(1280);
...
// Constructor
// vessel = our parent vessel
XR5LowerInstrumentPanel1280::XR5LowerInstrumentPanel1280(XR5Vanguard &vessel) : 
    XR5LowerInstrumentPanel(vessel, IDB_PANEL3_1280)
{
    AddCommonAreas(1280);
...
// Constructor
// vessel = our parent vessel
XR5MainInstrumentPanel1600::XR5MainInstrumentPanel1600(XR5Vanguard &vessel) : 
    XR5MainInstrumentPanel(vessel, IDB_PANEL1_1600)
{
    // create our components
    AddComponent(new MFDComponent                 (*this, _COORD2(   0, 242), MFD_LEFT));
    ...
    AddComponent(new XR5WarningLightsComponent    (*this, _COORD2(1360, 159)));
...
// Constructor
// vessel = our parent vessel
XR5UpperInstrumentPanel1600::XR5UpperInstrumentPanel1600(XR5Vanguard &vessel) : 
    XR5UpperInstrumentPanel(vessel, IDB_PANEL2_1600)
{
...
// Constructor
// vessel = our parent vessel
XR5LowerInstrumentPanel1600::XR5LowerInstrumentPanel1600(XR5Vanguard &vessel) : 
    XR5LowerInstrumentPanel(vessel, IDB_PANEL3_1600)
{
    const int width = 1600;
...
// Constructor
// vessel = our parent vessel
XR5MainInstrumentPanel1920::XR5MainInstrumentPanel1920(XR5Vanguard &vessel) : 
    XR5MainInstrumentPanel(vessel, IDB_PANEL1_1920)
{
    // create our components
    AddComponent(new MFDComponent                 (*this, _COORD2(   0, 242), MFD_LEFT));
    ...
    AddComponent(new XR5WarningLightsComponent    (*this, _COORD2(1680, 159)));
...
// Constructor
// vessel = our parent vessel
XR5UpperInstrumentPanel1920::XR5UpperInstrumentPanel1920(XR5Vanguard &vessel) : 
    XR5UpperInstrumentPanel(vessel, IDB_PANEL2_1920)
{
...
// Constructor
// vessel = our parent vessel
XR5LowerInstrumentPanel1920::XR5LowerInstrumentPanel1920(XR5Vanguard &vessel) : 
    XR5LowerInstrumentPanel(vessel, IDB_PANEL3_1920)
{
    AddCommonAreas(1920);
...
// glass cockpit
bool XR5Vanguard::clbkLoadGenericCockpit()
{
