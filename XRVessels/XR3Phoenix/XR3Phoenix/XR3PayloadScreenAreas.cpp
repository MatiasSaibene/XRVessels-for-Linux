...
bool SelectPayloadSlotArea::Redraw2D(const int event, const SURFHANDLE surf)
{
    ...
    if (!GetXR3().m_grappleTargetVesselName != 0)  // anything there?
    {
        ...
        OBJHANDLE hTarget = oapiGetVesselByName(const_cast<char *>(GetXR3().m_grappleTargetVesselName));   // will be nullptr if vessel no longer in range
        if (hTarget != nullptr)
        {
...
bool SelectPayloadSlotArea::ProcessMouseEvent(const int event, const int mx, const int my)
{
        ...
        (my >= 5)  && (my <= 126))
    {
        ...
        XRPayloadBaySlot *pSlot = GetXR3().m_pPayloadBay->GetSlotForGrid(GetXR3().m_selectedSlotLevel, gridX, gridY);  // will be nullptr if no slot at requested coordinates
        if (pSlot != nullptr)
        {
